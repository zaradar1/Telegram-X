...
def save_user_credentials(user_id, api_id, api_hash, session_string):
    api_hash_enc = cipher_suite.encrypt(str(api_hash).encode()).decode()
    session_enc = cipher_suite.encrypt(str(session_string).encode()).decode()
    with get_db() as conn:
        conn.execute('''
            INSERT INTO user_credentials (user_id, api_id, api_hash_encrypted, session_string_encrypted)
            VALUES (?, ?, ?, ?)
        ...
        ''', (user_id, str(api_id), api_hash_enc, session_enc))
...
def get_user_credentials(user_id):
    with get_db() as conn:
        ...
        if row and row['session_string_encrypted']:
            try:
                api_hash = cipher_suite.decrypt(row['api_hash_encrypted'].encode()).decode()
                session_string = cipher_suite.decrypt(row['session_string_encrypted'].encode()).decode()
                return row['api_id'], api_hash, session_string
            except Exception as e:
...
def register_handlers(b: telebot.TeleBot) -> None:
    ...
    def cmd_start(message):
        ...
        if uid in ADMIN_IDS:
            markup.add(
                KeyboardButton('/scraper'), KeyboardButton('/download'),
                KeyboardButton('/forward'), KeyboardButton('/status'),
